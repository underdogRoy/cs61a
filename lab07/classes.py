    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand)>1:
            ...
            for i in range(len(player.hand)):
                if player.hand[i].power(opponent_card)>player.hand[max].power(opponent_card):
                    max=i
            self.attack+=player.hand[max].attack
            self.defense+=player.hand[max].defense
            best_card=player.hand.pop(max)
        '''if len(player.hand) > 1:
