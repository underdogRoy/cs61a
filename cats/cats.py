...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words),len(source_words))):
        if typed_words[i]==source_words[i]:
            same_part+=1
    return 100*same_part/len(typed_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    difference=[diff_function(typed_word,word,limit)for word in word_list]
    if min(difference)<=limit:
        return word_list[difference.index(min(difference))]
    else:
        return typed_word
...
def feline_fixes(typed, source, limit):
    ...
    count += abs(len(source) - len(typed))
...
def time_per_word(words, times_per_player):
    ...
    for i in times_per_player:
        list_times_per_player=[]
        for j in range(1,len(i)):
            list_times_per_player.append(i[j]-i[j-1])
        times.append(list_times_per_player)
    ...
    for j in word_indices:
        ...
        for i in player_indices:
            if match["times"][i][j] < min_time:
                ...
                min_index = i
        fastest_words_list[min_index].append(match["words"][j])
